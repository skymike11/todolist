...
class ItemList extends Component {
    ...
    operateCell = (text, recorder) => {
        switch (recorder.status) {
            case false :
                return (
                    <div><Button type="primary" shape="round" size={"small"} onClick={() => {
                        this.handleFinishItem(recorder)
                    }}> Done</Button> <Button type="primary" shape="round" size={"small"}
                                              onClick={() => {
                                                  this.handleDeleteItem(recorder)
                                              }}>Delete</Button></div>);
            default:
                return (<Button type="primary" shape="round" size={"small"} onClick={() => {
                    this.handleDeleteItem(recorder)
                }}>Delete</Button>);
        }
    };
    ...
    render() {
        return <div style={{alignItems: "center", marginTop: "50px"}}>
            <Table dataSource={this.props.items} style={{width: "28%", margin: "0 auto"}}>
                <Column title='Id' dataIndex='backupNo' render={(text, recorder, index) => <span>{index + 1}</span>}/>
                <Column title='Content' dataIndex='content' style={{width: "90%"}}/>
                <Column title='Status' dataIndex='status'
                        render={(text, recorder) => {
                            if (recorder.status === true) {
                                return (
                                    <Tag color={"green"} key={"Finish"}>Finish</Tag>
                                )
                            } else {
                                return (
                                    <Tag color={"red"} key={"UnFinish"}>UnFinish</Tag>
                                )
                            }
                        }}/>
                <Column title='Options' dataIndex='operate'
